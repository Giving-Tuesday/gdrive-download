...
class AarTemplate(DocumentTemplate):
    ...
    def section_headers(self) -> Dict[str, List[str]]:
        ...
        return {
            'challenges': [
                ...
                'areas for improvement', 'lessons learned', 'what didn\'t work',
                'obstacles', 'barriers', 'pain points'
            ],
            'successes': [
                'successes', 'wins', 'what went well', 'achievements',
                'positive outcomes', 'strengths', 'accomplishments',
                'highlights', 'good news', 'what worked'
            ],
            'lessons': [
                'lessons learned', 'key learnings', 'takeaways',
                'insights', 'learnings', 'key insights', 'lessons'
            ],
            'recommendations': [
                'recommendations', 'next steps', 'action items',
                'improvements', 'suggestions', 'future actions'
            ]
        }
    ...
    def preprocess_document(self, content: str, metadata=None) -> str:
        ...
        for phrase_pattern in template_phrases:
            content = re.sub(phrase_pattern, "", content, flags=re.IGNORECASE | re.DOTALL)
        ...
        for old_header, new_header in header_replacements.items():
            ...
            pattern = r"^\s*#{1,6}\s*" + escaped_header + r"|^\s*" + escaped_header + r":"
            content = re.sub(
                pattern,
                f"# {new_header}",
                content,
    ...
    def extract_themes(self, matches: Dict[str, List[Dict]]) -> Dict[str, List[str]]:
        ...
        if 'challenges' in matches:
            ...
            for pattern, count in sorted_patterns[:5]:  # Top 5 themes
                if count >= 2:  # Must appear at least twice
                    challenge_themes.append(f"{pattern.replace('_', ' ').title()} (mentioned {count} times)")
        ...
        if 'successes' in matches:
            ...
            for pattern, count in sorted_patterns[:5]:
                if count >= 2:
                    success_themes.append(f"{pattern.replace('_', ' ').title()} (mentioned {count} times)")
