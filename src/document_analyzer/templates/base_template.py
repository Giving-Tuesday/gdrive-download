...
class DocumentTemplate(ABC):
    ...
    def preprocess_document(self, content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
    ...
    def extract_sections(self, content: str) -> Dict[str, str]:
        ...
        for section_name, headers in self.section_headers.items():
            ...
            for header in headers:
                ...
                pattern = re.compile(r'^\s*#{1,6}\s*' + re.escape(header) + r'|^\s*' + re.escape(header) + r':', 
                ...
                for match in matches:
                    ...
                    next_header_pattern = re.compile(r'^\s*#{1,6}\s+\w+|^\s*\w+:', re.MULTILINE)
    ...
    def postprocess_matches(self, matches: Dict[str, List[Dict[str, Any]]]) -> Dict[str, List[Dict[str, Any]]]:
        """
...
def load_template(template_name: str) -> DocumentTemplate:
    ...
    try:
        ...
        module = importlib.import_module(f'document_analyzer.templates.{template_name}')
        ...
        if hasattr(module, template_class_name):
            ...
            raise AttributeError(f"Template class '{template_class_name}' not found in module")
...
def list_available_templates() -> List[str]:
    ...
    for template_file in templates_dir.glob("*.py"):
        if template_file.name.startswith("_") or template_file.name == "base_template.py":
            continue
