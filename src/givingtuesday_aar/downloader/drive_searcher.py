...
class GoogleDriveSearcher:
    ...
    def search_files(
        ...
        if drive_scope == "personal":
            results.extend(self._search_drive(query, None, max_results, "My Drive", since_date))
        elif drive_scope == "shared" and shared_drive_id:
            results.extend(self._search_drive(query, shared_drive_id, max_results, "Shared Drive", since_date))
        elif drive_scope == "all":
            # Search personal drive
            console.print("[blue]Searching My Drive...[/blue]")
            personal_results = self._search_drive(query, None, max_results, "My Drive", since_date)
            ...
            try:
                ...
                for drive in track(all_shared_drives, description="Searching shared drives"):
                    ...
                    drive_results = self._search_drive(
                        query, 
                        drive['id'], 
                        max_results - len(results),  # Only get remaining needed results
                        drive['name'],
                        since_date
                    )
                    results.extend(drive_results)
            except HttpError as e:
                console.print(f"[yellow]Warning: Could not list shared drives: {e}[/yellow]")
    ...
    def create_shortcuts(self, results: List[Dict[str, str]], target_folder_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            console.print(f"[yellow]Warning: Could not check existing shortcuts: {e}[/yellow]")
